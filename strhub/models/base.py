...
class BatchResult:
    ...
    loss: Tensor
    loss_numel: int
    # --- Add the following fields for returning prediction comment out after use ---
    preds: List[str] #comment out after use
    gts: List[str]#comment out after use
    confidences: List[float]#comment out after use
...
class BaseSystem(pl.LightningModule, ABC):
    ...
    #         cer += edit_distance(pred, gt) / len(gt)#initially given as crr changed to cer
    #         if pred == gt:
    #             correct += 1
    #         total += 1
    #         label_length += len(pred)
    #     return dict(output=BatchResult(total, correct, ned, cer, confidence, label_length, loss, loss_numel))

    
    #comment out this function after use 
    ...
    def _eval_step(self, batch, validation: bool) -> Optional[STEP_OUTPUT]:
        ...
        # Create lists to store batch-level results
        batch_preds = []
        batch_confidences = []
        ...
        for pred, prob, gt in zip(preds, probs, labels):
            # Store per-sample confidence
            sample_confidence = prob.prod().item()
            batch_confidences.append(sample_confidence)
        ...
        # Populate the enhanced BatchResult object
        return dict(output=BatchResult(total, correct, ned, cer, confidence, label_length, loss, loss_numel,
                                       preds=batch_preds, gts=labels, confidences=batch_confidences))
    @staticmethod
    ...
    #added: 28th jan 2025
    #added: added generalized_test which returns image, image_name, predicted label, confidence score for the batch 
    def generalized_test(self, batch) -> Tuple[list,list]:
        images, labels = batch
        ...
        preds, probs = self.tokenizer.decode(probs)
        for pred, prob, gt in zip(preds, probs, labels):
            confidence.append(prob.prod().item())
            pred = self.charset_adapter(pred)
