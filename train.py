...
def main(config: DictConfig):
    ...
    if config.pretrained is not None:
        ...
        model = load_from_checkpoint(config.pretrained)  # Automatically handles local checkpoints

    print(summarize(model, max_depth=1 if model.hparams.name.startswith('parseq') else 2))

    datamodule: SceneTextDataModule = hydra.utils.instantiate(config.data)

    checkpoint = ModelCheckpoint(monitor='val_loss', mode='min', save_top_k=5, save_last=True,
